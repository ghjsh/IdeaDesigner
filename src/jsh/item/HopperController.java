...
public class HopperController extends ItemController<Hopper> {
    ...
    public void initialize_() {
        ...
        if (item.getResult() == null) item.setResult(new Content("제목", "내용"));
        Bindings.bindBidirectional(resultTitle.textProperty(), item.getResult().getTitle());
        Bindings.bindBidirectional(resultContent.textProperty(), item.getResult().getContent());
    }
}
